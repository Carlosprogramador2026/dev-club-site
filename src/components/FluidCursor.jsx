...
// Trilha de fluido seguindo o cursor — simulacao WebGL2 (Navier-Stokes
// simplificado), adaptada de um componente Framer ("Fluid Trail —
// Originkit") pro site. So' roda em dispositivos com mouse de verdade
// (hover: hover) e respeita prefers-reduced-motion.

// Parseia uma cor CSS ("#rgb", "#rrggbb", "rgb(...)", "rgba(...)") pra 0-1 RGB.
...
// Configuracao fixa do efeito (cor = verde neon da marca).
const CONFIG = {
  color: "#00ffa3",
  mouseRadius: 12,
  trailDuration: 6.5,
...
export default function FluidCursor() {
  ...
  useEffect(() => {
    // So' em telas com mouse de verdade, e so' se o usuario nao pediu
    // pra reduzir animacoes.
    ...
    if (!gl) {
      console.warn("FluidCursor: WebGL2 indisponivel")
      return
    }
    ...
    // Escuta na window (nao no canvas) — o canvas fica com pointer-events:
    // none pra nao bloquear cliques no resto do site.
    ...
    function splatAt(x, y, dx, dy, color) {
      ...
      const mr = Math.max(1, CONFIG.mouseRadius)
    ...
    function applyPointerInput() {
      ...
      const c = parseColor(CONFIG.color)
      const col = [c[0] * 0.7, c[1] * 0.7, c[2] * 0.7]
    ...
    function step(dt) {
      ...
      gl.uniform1f(gl.getUniformLocation(pAdvect, "u_dt"), dt)
      // Trail Duration em segundos -> decaimento exponencial do dye.
      const dur = Math.max(0.1, CONFIG.trailDuration)
      const dyeDiss = 3 / dur
      gl.uniform1f(gl.getUniformLocation(pAdvect, "u_dissipation"), 1 - dyeDiss * dt)
      const fadeOut = CONFIG.fade === "outside"
      gl.uniform1f(gl.getUniformLocation(pAdvect, "u_outward"), fadeOut ? 0.6 * dt : 0)
      blit(dye.write)
      dye.swap()
    }
